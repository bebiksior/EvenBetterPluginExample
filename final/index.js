var D=new Set,p=({id:_,cssText:Z})=>{if(D.has(_)||document.querySelector(`#${_}`))return;const q=document.createElement("style");q.id=_,q.textContent=Z,q.classList.add("evenbetterapi-css-module"),document.head.appendChild(q),D.add(_)},Y=p;var s=`
.eb-checkbox__input {
    cursor: pointer;
...
}`,n=(_,Z,q)=>{const K=document.createElement("div");K.classList.add("v-toast"),K.classList.add(`v-toast--${q}`);const z=document.createElement("div");z.setAttribute("role","alert"),z.classList.add("v-toast__item"),z.classList.add(`v-toast__item--${Z}`),z.classList.add(`v-toast__item--${q}`),K.appendChild(z);const F=document.createElement("div");F.classList.add("v-toast__icon"),z.appendChild(F);const G=document.createElement("p");return G.classList.add("v-toast__text"),G.textContent=_,z.appendChild(G),K.classList.add("v-toast--fade-in"),K},I=(_)=>{const{message:Z,type:q,position:K,duration:z}=_;Y({id:"eb-toast",cssText:o});let F=document.querySelector(`.v-toast--${K}`);if(!F)F=document.createElement("div"),F.classList.add("v-toast"),F.classList.add(`v-toast--${K}`),document.body.appendChild(F);const G=n(Z,q,K);F.appendChild(G),setTimeout(()=>{G.classList.add("v-toast--fade-out"),setTimeout(()=>{G.remove()},150)},z-150)};function w(_,Z="Search...",q=!1,K){Y({id:"eb-text-input",cssText:t});const z=document.createElement("div");z.classList.add("formkit-outer","c-text-input__outer"),z.setAttribute("style",`width: ${_};`);const F=document.createElement("div");F.classList.add("formkit-wrapper"),F.style.display="flex";const G=document.createElement("div");G.classList.add("formkit-inner","eb-text-input__inner");const H=document.createElement("div");H.classList.add("c-text-input__prefix"),H.setAttribute("style","align-self: center; color: var(--c-fg-subtle);");const J=document.createElement("i");if(J.classList.add("c-icon","fas"),K)J.classList.add(K);const Q=document.createElement("input");if(Q.setAttribute("placeholder",Z),Q.setAttribute("spellcheck","false"),Q.classList.add("formkit-input","c-text-input__input"),Q.setAttribute("type","text"),Q.setAttribute("style","width: 100%; background: transparent; border: 0; outline: 0; color: inherit; box-sizing: border-box; line-height: inherit;"),K)H.appendChild(J);if(G.appendChild(H),G.appendChild(Q),q){const j=document.createElement("button");j.innerHTML='<i class="fas fa-copy"></i>',j.setAttribute("style","background: transparent; border: 0; outline: 0; cursor: pointer; padding: 0; margin-left: 10px;"),j.addEventListener("click",()=>{navigator.clipboard.writeText(Q.value),X.toast.showToast({message:"Copied to clipboard",position:"bottom",type:"info",duration:1000})}),G.appendChild(j)}return F.appendChild(G),z.appendChild(F),z}var t=`
